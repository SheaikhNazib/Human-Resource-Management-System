...
export async function createLeave(leaveData) {
  try {
    ...
    if (responseData?.statusCode && responseData.statusCode >= 400) {
      const errorMsg =
        responseData.message || responseData.error || "Failed to create leave";
      console.error(
    ...
    if (!responseData?.id) {
      const errorMsg =
        ...
        responseData?.error ||
        "Leave created but no ID returned";
...
export async function updateLeave(id, leaveData) {
  try {
    ...
    if (responseData?.statusCode && responseData.statusCode >= 400) {
      const errorMsg =
        responseData.message || responseData.error || "Failed to update leave";
      return { success: false, error: errorMsg };
    }
...
export async function updateLeaveStatus(id, status, approvedBy = null) {
  try {
    ...
    if (responseData?.statusCode && responseData.statusCode >= 400) {
      const errorMsg =
        responseData.message || responseData.error || "Failed to update status";
      return { success: false, error: errorMsg };
    }
