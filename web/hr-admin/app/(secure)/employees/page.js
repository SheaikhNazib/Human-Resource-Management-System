  ...
  const columns = [
    {
      ...
      render: (emp) => {
        ...
        const last =
          ...
          "";
        const fullName =
          `${first} ${last}`.trim() || emp.name || emp.raw?.name || "—";
        const email =
          emp.email ??
          emp.raw?.email ??
          emp.raw?.email_address ??
          emp.raw?.emailAddress ??
          "—";
        const initials =
          (first?.[0] || fullName?.[0] || "").toUpperCase() +
          (last?.[0] || "").toUpperCase();
    ...
    },
    {
      ...
      accessor: "department",
      render: (emp) =>
        emp.department ?? emp.raw?.department ?? emp.raw?.dept ?? "—",
    },
    {
      header: "Status",
      accessor: "status",
